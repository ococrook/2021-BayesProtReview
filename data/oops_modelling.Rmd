...
It is first a good idea to get a feel for the data using some exploratory data
...
```
Now we perform PCA to see if there is an systematic clustering. There
...
slightly higher variance for the oops samples
```{r,}
...
looks like following for each protein. 
$$
y = total + time + total*time
$$
...
on the same scale as type changes. One would expect that after account for
...
for an decrease in the proprotion of this protein binding RNA relative to total
